...
int main(int argc, char** argv) {
  ...
  for (auto it = view.begin(); it != view.end(); ++it) {
    ...
    if (it->isType<poslv::VehicleNavigationSolutionMsg>()) {
      ...
      navigationMeasurements.push_back(std::make_pair(vns->header.stamp.toSec(),
        data));
    }
    ...
    if (it->isType<poslv::TimeTaggedDMIDataMsg>()) {
      ...
      encoderMeasurements.push_back(std::make_pair(dmi->header.stamp.toSec(),
        data));
    }
  }
  ...
  const int measPerSecDesired = 10;
...
//    bspdv->designVariable(i)->setActive(true);
  ...
  auto dmidv = boost::make_shared<VectorDesignVariable<1> >(
    (VectorDesignVariable<1>::Container() << e_r).finished());
  dmidv->setActive(true);
  problem->addDesignVariable(dmidv);
...
//  C_io_dv->setActive(true);
  ...
  for (auto it = encoderMeasurements.cbegin(); it != encoderMeasurements.cend();
    ...
    if (lastTimestamp != -1) {
      ...
      auto e_dmi = boost::make_shared<ErrorTermDMI>(v_oo, om_oo,
        dmidv.get(), meas,
...
//      problem->addErrorTerm(e_dmi);
  ...
  std::cout << "CAN intrinsic: " << *cpdv << std::endl;
  std::cout << "DMI intrinsic: " << *dmidv << std::endl;
