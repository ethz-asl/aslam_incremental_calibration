...
//    Eigen::Matrix<double, 3, 3>::Identity() * 1e-2).getSample();
  ...
  for (size_t i = 0; i < steps; ++i) {
    ...
    problem->addDesignVariable(dv_x[i]);
  }
  ...
  for (size_t i = 0; i < nl; ++i) {
    ...
    problem->addDesignVariable(dv_x_l[i]);
  }
  ...
  for (size_t i = 1; i < steps; ++i) {
    boost::shared_ptr<aslam::calibration::ErrorTermMotion> e_mot(
      new aslam::calibration::ErrorTermMotion(dv_x[i - 1].get(), dv_x[i].get(),
      T, u_noise[i], Q));
    problem->addErrorTerm(e_mot);
    for (size_t j = 0; j < nl; ++j) {
      boost::shared_ptr<aslam::calibration::ErrorTermObservation> e_obs(
        new aslam::calibration::ErrorTermObservation(dv_x[i].get(),
