...
cholmod_sparse* columnSubmatrix(cholmod_sparse* A, std::ptrdiff_t
  ...
  CHECK_LT(colEndIdx, colStartIdx) << "end must be lower than start index.";
  CHECK_GE(colEndIdx, static_cast<std::ptrdiff_t>(A->ncol))
   << "index must be lower than the number of columns";
